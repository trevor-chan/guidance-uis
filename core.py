...
def random_study_target(box_origin: np.ndarray) -> np.ndarray:
    ...
    BOX_Z_MIN–BOX_Z_MAX depth) expressed in world frame via the box basis.

    Orientation: intrinsic XYZ perturbation of the set-box orientation —
    roll ∈ [−π, π), pitch ∈ [−60°, 60°], yaw ∈ [−60°, 60°].
    Composition order matches pose_math.component_errors decomposition:
    R_perturb = Rx(roll) @ Ry(pitch) @ Rz(yaw).
    """
